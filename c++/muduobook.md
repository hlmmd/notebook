...
在业务线程中使用IO操作可能会导致阻塞等情况，影响业务的正常运行

使用BlockingQueue：每次产生日志都会通知对方，效率较低。

muduo采用双缓冲技术。A用来在前段接受其他线程的日志信息，B用来将日志写到磁盘上。可以将多条日志拼成一个大的buffer送给后端，相当于批处理，减少线程唤醒次数。A满后与B交换，B写入磁盘。在此基础上设置定时器，每3s固定执行一次上述操作。
